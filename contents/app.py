    ...
    if (index == -1):
        ...
        if 'list' in content and content['list'] is not None:
            if (len(content['list']) == 0):
                contents = [x for x in components[index]["contents"] if str(x['id']) != str(content["id"])]
...
async def get_props_by_id_component(idComponent: str, content: Content):
    ...
    if (index == -1):
        ...
        if 'contents' in components[index] and components[index]['contents'] is not None:
...
async def delete_content_list(idComponent: str, idContent: str):
    ...
    if (index == -1):
        ...
        if 'contents' in components[index] and components[index]['contents'] is not None:
            contents = [x for x in components[index]['contents'] if str(x['id']) != str(idContent)]
            print(contents)
            components[index]['contents'] = contents
